    ...
    const fetchData = async () => {
      try {
        ...
        if (cnClub) {
          ...
          // 3. Get Openings
          const openingRes = await fetch(`/api/openings/${cnClub._id}`);
          if (openingRes.ok) {
        ...
        <div className="bg-black bg-opacity-40 p-8 rounded-2xl shadow-xl backdrop-blur-lg border border-white border-opacity-20 max-w-4xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            <div className="text-center md:text-left mt-4">
              <h1 className="text-6xl font-extrabold text-orange-500 mb-3">
                Coding Ninjas
              </h1>
              <p className="text-2xl text-gray-200 mb-4">
                The Coding Ninjas Student Chapter
              </p>
              <div className="flex justify-center md:justify-start space-x-4">
                <a
                  href="https://www.instagram.com/codingninjas_cuiet?igsh=MWVrbGxrOWxwc2Frag=="
                  target="_blank"
          ...
          {opening && (opening.technicalRoles?.length > 0 || opening.nonTechnicalRoles?.length > 0) && (
            <div className="mt-12 relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-orange-600 to-red-600 rounded-xl blur opacity-25 group-hover:opacity-75 transition duration-1000 group-hover:duration-200 animate-pulse"></div>
              <div className="relative bg-black bg-opacity-80 p-8 rounded-xl border border-orange-500/50 shadow-[0_0_30px_rgba(249,115,22,0.3)]">
                <div className="flex flex-col md:flex-row items-start justify-between gap-8">
                  <div className="flex-1 text-center md:text-left">
                    <h2 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-white mb-4 animate-pulse">
                      WE ARE HIRING!
                    </h2>
                    ...
                    <div className="space-y-4">
                      {opening.technicalRoles?.length > 0 && (
                        <div>
                          <h3 className="text-orange-400 font-bold mb-2 uppercase tracking-wider text-sm">Technical</h3>
                          <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                            {opening.technicalRoles.map(role => (
                              <span key={role} className="px-3 py-1 bg-orange-900/30 border border-orange-500/30 rounded-full text-orange-200 text-sm">
                                {role}
                              </span>
                            ))}
                          </div>
          ...
          <div className="mt-12 border-t border-gray-600 pt-8">
            ...
            <p className="text-lg text-gray-300 leading-relaxed">
              Sharpen coding skills with contests, projects, and mentorship. We bridge the gap between academic learning and industry standards, focusing on DSA, Web Dev, and more.
            </p>
          </div>
        </div>
      </main>
